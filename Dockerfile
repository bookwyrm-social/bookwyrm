...
RUN apt-get update && apt-get install -y gettext libgettextpo-dev tidy libpq5 libsass1 libsass-dev && apt-get clean
...
RUN apt-get update && apt-get install -y gettext tidy libpq5 libsass1 curl && apt-get clean
ENV PYTHONUNBUFFERED=1
ENV PATH=/venv/bin:$PATH
ENV SYSTEM_SASS=True
