...
- `docker-compose exec web poetry run python manage.py migrate` runs the database migrations in Django
- `docker-compose exec web poetry run python manage.py collectstatic --no-input` loads any updated static files (such as the JavaScript and CSS)
